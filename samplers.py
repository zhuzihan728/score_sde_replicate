...
def get_sampler(sde, score_fn, config):
    ...
    shape = (local_batch_size, config.data.image_size, config.data.image_size, config.data.num_channels)
    ...
    if config.sampler.type == 'pc':
        ...
        if config.sampler.corrector == 'Langevin':
            corrector = LangevinCorrector(sde, score_fn, config.sampler.corrector_snr, config.sampler.corrector_steps)
        elif config.sampler.corrector == 'AnnealedLangevin':
            corrector = AnnealedLangevinCorrector(sde, score_fn, config.sampler.corrector_snr, config.sampler.corrector_steps)
        else:
            corrector = Corrector()
        return pc_sampler(sde, shape, predictor, corrector, inverse_scaler, config.sampler.sampler_steps, config.sampler.denoise)
    else:
        return ode_sampler(sde, score_fn, shape, inverse_scaler, config.sampler.denoise, config.sampler.rtol,
...
def pc_sampler(sde, shape, predictor, corrector, inverse_scaler, n_steps=1_000, denoise=True, epsilon=1e-3):
    def sampler_batch(rng):
        ...
        def iteration(step, val):
            ...
            if isinstance(predictor, (EulerMaruyamaPredictor, ReverseDiffusionPredictor,
                                      AncestralSamplingPredictor, ProbabilityFlowPredictor)):
                rng, step_rng = jax.random.split(rng)
...
def ode_sampler(sde, score_fn, shape, inverse_scaler, denoise=False,
    ...
    def sampler(rng, z=None):
        ...
        def ode_fn(t, x_flat):
            drift = drift_fn(x_flat.reshape(shape), jnp.full((shape[0],), t))
            return np.array(drift).reshape(-1)

        solution = integrate.solve_ivp(ode_fn, (sde.T, eps), jnp.array(x).reshape(-1), rtol=rtol, atol=atol, method=method)
        ...
        return inverse_scaler(x), solution.nfev
